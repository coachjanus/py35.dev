...
def create():
   ...
   if request.method == 'POST':
       ...
       if role_id in (1,2,3,4):
           db.execute(
               "INSERT INTO employee(employee_name, department_id, role_id, weekly_salary, commission_per_sale, hour_rate) VALUES(?, ?, ?, ?, ?, ?)", (employee_name, department_id, role_id, weekly_salary, commission_per_sale, hour_rate)
           )
           db.commit()
       else:
...
def get_employee(id):
    employee = get_db().execute('SELECT * FROM employee WHERE id = ?', (id,)).fetchone()
    if employee is None:
...
def update(id):
    ...
    departments = get_db().execute('SELECT id, department_name FROM department').fetchall()
    ...
    if request.method == 'POST':
        ...
        if error is not None:
            ...
            db = get_db()
            db.execute('UPDATE employee SET employee_name = ?, department_id = ?, role_id = ?,weekly_salary=?, commission_per_sale=?, hour_rate=? WHERE id = ?', (employee_name,department_id, role_id, weekly_salary, commission_per_sale, hour_rate, id) )
            db.commit()
            return redirect(url_for('staff.index'))
